...
'输入仪器名称
For i = 1 To YqV
    mn = Split(Cells(13, 2 + (i - 1) * NumYK_V), "_")
    Yqm(i - 1) = mn(2) & mn(3)
  
Next i

For i = 1 To YqD
    mn = Split(Cells(13, 2 + YqV * NumYK_V + (i - 1) * NumYK_D), "_")
    Yqm(YqV + i - 1) = mn(2) & mn(3)
Next i
...
Dlow = 0 '风向最小值
Dtop = 360.1 '风向最大值
Plow = 94 / (10 ^ (X_HB / 18400 * (1 + (1 / 273) * (X_Tem + X_HB / 200 * 0.6)))) * 10 '气压最小值hPa/mbar， 假设海拔升高100米气温降低0.6℃，压高公式
Ptop = 106 / (10 ^ (X_HB / 18400 * (1 + (1 / 273) * (X_Tem + X_HB / 200 * 0.6)))) * 10 '气压最大值hPa/mbar
Tlow = -40 '温度最小值摄氏度
Ttop = 50 '温度最大值摄氏度
...
Fanw(i, 1) = Vtop
Vspeed(i) = i * NumYK_V + 2 '风速所在列
HVgao(i) = CInt(Replace(Replace(Replace(Replace(Replace(Yqm(i), "m", ""), "W", ""), "N", ""), "E", ""), "S", ""))  '风速高
Next i

For i = 0 To YqD - 1
Fanw(i + YqV, 0) = Dlow   '风向合理范围
Fanw(i + YqV, 1) = Dtop
Ddirect(i) = NumYK_V * YqV + i * NumYK_D + 2  '风向所在列
HDgao(i) = CInt(Replace(Replace(Replace(Replace(Replace(Yqm(i + YqV), "m", ""), "W", ""), "N", ""), "E", ""), "S", "")) 'CInt(Left(Yqm(i + YqV), 2))   '风向所在高度
Next i

For i = 0 To YqT - 1
Fanw(i + YqV + YqD, 0) = Tlow    '温度合理范围
Fanw(i + YqV + YqD, 1) = Ttop
YqTP(i) = NumYK_V * YqV + NumYK_D * YqD + i + 2 '温度所在列  *****注意温度气压的顺序
YqTPm(i) = Yqm(YqV + YqD)
Next i
...
YqTP(YqT + i) = NumYK_V * YqV + NumYK_D * YqD + NumYK_TP * YqT + 2 '气压所在列
...
Cells(Z_SZH_Qrow, Z_SZH_QColumn) = dtodfm(CSng(Split(Range("a3"), " ")(3)))
Range(Cells(Z_SZH_Qrow + 2, Z_SZH_QColumn), Cells(Z_SZH_Qrow + 3, Z_SZH_QColumn)).Merge
Cells(Z_SZH_Qrow + 2, Z_SZH_QColumn) = dtodfm(CSng(Split(Range("a4"), " ")(3)))
...
Cells(Z_SZH_Qrow + 2, Z_SZH_QColumn + 3) = "气温：" & Replace(Split(Cells(13, YqTP(0)), "_")(2), "m", "")
Cells(Z_SZH_Qrow + 3, Z_SZH_QColumn + 3) = "气压：" & Replace(Split(Cells(13, YqTP(1)), "_")(2), "m", "")
...
For i = YqV To YqV + YqD - 1
col = NumYK_V * YqV + (i - YqV) * NumYK_D + 2
Cells(Z_F_Qrow + 1, Z_F_Qcolumn + i + 1).Value = 单列完整年范围检查(col, Fanw(i, 0), Fanw(i, 1), Snum)
Next

For i = YqV + YqD To YqV + YqD + YqT + YqP - 1
col = NumYK_V * YqV + YqD * NumYK_D + (i - YqV - YqD) * NumYK_TP + 2
Cells(Z_F_Qrow + 1, Z_F_Qcolumn + i + 1).Value = 单列完整年范围检查(col, Fanw(i, 0), Fanw(i, 1), Snum)
Next
