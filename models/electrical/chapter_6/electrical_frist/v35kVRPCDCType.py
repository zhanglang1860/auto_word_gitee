...
# 5.4 35kV无功补偿装置柜
class v35kVRPCDCType:

    def __init__(self):
        # ===========selecting parameters=============
        self.TypeID = 0
        # ===========basic parameters==============
        self.Data35kVRPCDCType = pd.DataFrame()
    ...
    def extraction_data_35kVRPCDCType_resource(self, TypeID):
        self.TypeID = TypeID

        sql = "SELECT * FROM auto_word_electrical_35kvrpcdctype"
        self.Data35kVRPCDCType = connect_sql_pandas(sql)
        self.Data35kVRPCDCType = \
            self.Data35kVRPCDCType.loc[
                self.Data35kVRPCDCType['TypeID'] == self.TypeID]
        return self.Data35kVRPCDCType
    ...
    def generate_dict_35kVRPCDCType_resource(self, data, num):
        self.Data35kVRPCDCType = data
        self.numbers = num
        self.TypeName = self.Data35kVRPCDCType.at[self.Data35kVRPCDCType.index[0], 'TypeName']
        self.RatedVoltage = self.Data35kVRPCDCType.at[self.Data35kVRPCDCType.index[0], 'RatedVoltage']
        self.RatedCurrent = self.Data35kVRPCDCType.at[self.Data35kVRPCDCType.index[0], 'RatedCurrent']
        self.RatedBreakingCurrent = self.Data35kVRPCDCType.at[self.Data35kVRPCDCType.index[0], 'RatedBreakingCurrent']
        self.DynamicCurrent = self.Data35kVRPCDCType.at[self.Data35kVRPCDCType.index[0], 'DynamicCurrent']
        self.RatedShortTimeWCurrent = self.Data35kVRPCDCType.at[self.Data35kVRPCDCType.index[0], 'RatedShortTimeWCurrent']

        self.CurrentTransformerRatio = self.Data35kVRPCDCType.at[self.Data35kVRPCDCType.index[0], 'CurrentTransformerRatio']
        self.CurrentTransformerAccuracyClass = self.Data35kVRPCDCType.at[self.Data35kVRPCDCType.index[0], 'CurrentTransformerAccuracyClass']
        self.CurrentTransformerArrester = self.Data35kVRPCDCType.at[self.Data35kVRPCDCType.index[0], 'CurrentTransformerArrester']
     
        self.dict_35kVRPCDCType_resource = {
            'numbers_35kV无功补偿装置柜': int(self.numbers),
            '型号_35kV无功补偿装置柜': self.TypeName,
            '额定电压_35kV无功补偿装置柜': self.RatedVoltage,
            '额定电流_35kV无功补偿装置柜': self.RatedCurrent,
            '额定开断电流_35kV无功补偿装置柜': self.RatedBreakingCurrent,
            '动稳定电流_35kV无功补偿装置柜': self.DynamicCurrent,
            '额定短时耐受电流_35kV无功补偿装置柜': self.RatedShortTimeWCurrent,
            '电流互感器变比_35kV无功补偿装置柜': self.CurrentTransformerRatio,
            '电流互感器准确级_35kV无功补偿装置柜': self.CurrentTransformerAccuracyClass,
            '电流互感器避雷器_35kV无功补偿装置柜': self.CurrentTransformerArrester,
        }


        return self.dict_35kVRPCDCType_resource
